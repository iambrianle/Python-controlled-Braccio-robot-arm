...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct  9 15:24:13 2023
...
def rotate_joint(joint):  # rotate a specific joint to the outer limits of the joint angles
    ...
    def calculate_joint(joint, number):
        ...
        print(f'what is the joint angle  {angle_string_def_angles[joint[3]]}')
...
def write_position(theta_base=base[0], theta_shoulder=shoulder[0], theta_elbow=elbow[0], theta_wrist=wrist[0],
    ...
    theta_base_comp = solverNNA.backlash_compensation_base(
        theta_base)  # check if compensation is neededbacklash_compensation_base(theta_base)  #check if compensation is needed
...
def move_vertical(x, y):
    loop_iteration = np.linspace(0, 350, 3)
...
def move_horizontal(z):
    loop_iteration = np.linspace(100, 350, 4)
...
def pick_up(x, y):
    glass_pos = [300, 0]  # x,y pos of glass [310.95]
    delay = 2  # delay between steps original value 1
